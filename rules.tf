resource "aws_config_config_rule" "iam-password-policy" {
  ...
  input_parameters = <<EOF
   {
    "RequireUppercaseCharacters" : "${local.password_require_uppercase}",
    "RequireLowercaseCharacters" : "${local.password_require_lowercase}",
    "RequireSymbols" : "${local.password_require_symbols}",
    "RequireNumbers" : "${local.password_require_numbers}",
    "MinimumPasswordLength" : "${local.password_min_length}",
    "PasswordReusePrevention" : "${local.password_reuse_prevention}",
    "MaxPasswordAge" : "${local.password_max_age}"
  }
  EOF
...
resource "aws_config_config_rule" "acm-certificate-expiration-check" {
  ...
  input_parameters =  <<EOF
  {
    "daysToExpiration" : "${local.acm_days_to_expiration}"
  }
  EOF
